...
g = data.groupby('pid').size().sort_values(ascending=False).reset_index(name='Count') #Creating dataframe g that has PID in order of number of connections it has based on column 'Count'

f1 = open('out.txt', 'w')
dg = data.groupby('pid') #returns a dictionary with 'pid' as key and connections as values
for index, row in g.iterrows(): #iterating through g dataframe that has 'pid' and its 'Count'
  k = row['pid']
  print >>f1, dg.get_group(k) # returns groups based on key 'pid'
f1.close()
